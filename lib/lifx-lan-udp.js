...
const LifxLanUdp = function () {
	...
	this._send_times_for_req = 10;
	this._send_times_for_cast = 3;
	this._send_interval = 50; // msec
};
...
LifxLanUdp.prototype.init = function () {
	let promise = new Promise((resolve, reject) => {
		...
		this._udp.bind({ address: ip, port: this._UDP_PORT });
	});
	return promise;
};
...
LifxLanUdp.prototype._requestUnicast = function (p) {
	let promise = new Promise((resolve, reject) => {
		...
		if (p['ack_required'] || p['res_required']) {
			(async () => {
				for (let i = 0; i < this._send_times_for_req; i++) {
					await this._sendUdpPacket(buf, 0, buf.length, this._UDP_PORT, p['address']);
					if (this._requests[seq]) {
						await this._wait(this._send_interval);
					} else {
						break;
					}
				}
			...
			(async () => {
				for (let i = 0; i < this._send_times_for_cast; i++) {
					await this._sendUdpPacket(buf, 0, buf.length, this._UDP_PORT, p['address']);
					await this._wait(this._send_interval);
				}
				resolve();
...
LifxLanUdp.prototype._sendUdpPacket = function (msg, offset, length, port, address) {
	return new Promise((resolve, reject) => {
		this._udp.send(msg, offset, length, port, address, (error) => {
			if (error) {
				reject(error);
			} else {
				resolve();
...
LifxLanUdp.prototype._wait = function (msec) {
...
LifxLanUdp.prototype._sendBroadcast = async function (req_list) {
	this._udp.setBroadcast(true);
	for (let req of req_list) {
		let buf = req['buffer'];
		for (let i = 0; i < this._send_times_for_cast; i++) {
			await this._sendUdpPacket(buf, 0, buf.length, this._UDP_PORT, req['address']);
			await this._wait(this._send_interval);
		}
	}
};
